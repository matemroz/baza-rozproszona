...
public class SearchClientProfileServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (KontoValidator.validateIdRodzajuKonta(Integer
				.toString(idRodzajuKonta))) {
			rk = rkd.getDaneRodzajuKonta(idRodzajuKonta);
		}

		printClientPersonalAndAccountData(response, konto, klient, rk);
	}

	private void printClientPersonalAndAccountData(
			HttpServletResponse response, Konto konto, Klient klient,
			RodzajKonta rk) throws IOException {
		...
		WebAppUtil.printSimpleTableRow(writer, "Ulica zamieszkania",
				klient.getUlicaZamieszkania());
		WebAppUtil.printSimpleTableRow(writer, "Numer domu",
				klient.getNumerDomu());
		WebAppUtil.printSimpleTableRow(writer, "Numer mieszkania",
				klient.getNumerMieszkania());
		WebAppUtil.printSimpleTableRow(writer, "Numer telefonu",
				klient.getNumerTelefonu());
		WebAppUtil.printSimpleTableRow(writer, "Numer dowodu osobistego",
				klient.getNumerDowoduOsobistego());
		WebAppUtil.printSimpleTableRow(writer, "Numer paszportu",
				klient.getNumerPaszportu());
		writer.write("</table>");
		writer.write("<br><br><h3>Dane konta bankowego</h3>");
		writer.write("<table>");
		WebAppUtil.printSimpleTableRow(writer, "Numer konta",
				konto.getNrKonta());
		WebAppUtil.printSimpleTableRow(writer, "Dostępne środki",
				Double.toString(konto.getSrodki()));
		WebAppUtil.printSimpleTableRow(writer, "Oprocentowanie",
				Double.toString(rk.getIdRodzajuKonta()));
		WebAppUtil.printSimpleTableRow(writer, "Kapitalizacja",
				Integer.toString(rk.getKapitalizacja()));
